...
def tbptt(training: ExperimentData, 
    ...
    context_window: int, optional
        The number of steps (before the start of each subsequence) to be used as context.
    input_scaler : SequenceScaler, optional
    ...
    def sample_initial_instants(experiments: TensorExperimentData, 
                                Ts: int, 
                                N_seq: int,
                                context_window: int) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        context_window: int
            Number of steps (before the start of each subsequence) to be used as context.
        ...
        if (Ts < 0 or all([Ts >= t for t in T_exp])) and N_seq <= len(experiments):
            ...
            sampled_initial_inst = torch.zeros_like(sampled_experiments) + max(context_window, 0)
            return sampled_experiments, sampled_initial_inst

        # Range of admissible initial points
        T_range = [t - (Ts + max(context_window, 0)) for t in T_exp]
        ...
        for e, n_e in zip(*sampled_experiments.unique(return_counts=True), strict=True):
            ...
            sampled_initial_inst[sampled_experiments == e] = initial_indexes + max(context_window, 0)
    ...
    sampled_train_exp, sampled_train_idx = sample_initial_instants(train_list, Ts=Ts_train, N_seq=Ns_train, context_window=context_window)
    sampled_val_exp, sampled_val_idx = sample_initial_instants(val_list, Ts=Ts_val, N_seq=Ns_val, context_window=context_window)
    if testing:
        sampled_test_exp, sampled_test_idx = sample_initial_instants(test_list, Ts=Ts_test, N_seq=Ns_test, context_window=context_window)
    
    # Build:
    # - Training dataset (U_train, Y_train, X0_train)
    # - Validation dataset (U_val, Y_val, X0_val)
    # - Testing dataset, if any, (U_test, Y_test, X0_test)
    ...
    X0_train = torch.zeros(Ntr, context_window, n_out + n_in) if context_window > 0 else None
    X0_val = torch.zeros(Nva, context_window, n_out + n_in) if context_window > 0 else None
    ...
    for i, (exp, idx) in enumerate(zip(sampled_train_exp, sampled_train_idx, strict=True)):
        ...
        if context_window > 0:
            X0_train[i] = torch.cat([train_list[exp][0][idx-context_window:idx],
                                     train_list[exp][1][idx-context_window:idx]], dim=-1)
    ...
    for i, (exp, idx) in enumerate(zip(sampled_val_exp, sampled_val_idx, strict=True)):
        ...
        if context_window > 0:
            X0_val[i] = torch.cat([train_list[exp][0][idx-context_window:idx],
                                     train_list[exp][1][idx-context_window:idx]], dim=-1)

    train_tuple = (U_train, Y_train, X0_train) if context_window > 0 else (U_train, Y_train)
    val_tuple = (U_val, Y_val, X0_val) if context_window > 0 else (U_val, Y_val)

    if testing:
        Nte = sampled_test_exp.shape[0]
        Tte = min([t.shape[0] for t, _ in test_list]) - max(context_window, 0) if testing and Ts_test < 0 else Ts_test
        U_test = torch.zeros(Nte, Tte, n_in)
        Y_test = torch.zeros(Nte, Tte, n_out)
        X0_test = torch.zeros(Nte, context_window, n_in + n_out) if context_window > 0 else None
        ...
        for i, (exp, idx) in enumerate(zip(sampled_test_exp, sampled_test_idx, strict=True)):
            ...
            if context_window > 0:
                X0_test[i] = torch.cat([test_list[exp][0][idx-context_window:idx],
                                        test_list[exp][1][idx-context_window:idx]], dim=-1)
        
        test_tuple = (U_test, Y_test, X0_test) if context_window > 0 else (U_test, Y_test)

        return FullSequenceDataset(training=data.TensorDataset(*train_tuple), 
                                   validation=data.TensorDataset(*val_tuple),
                                   testing=data.TensorDataset(*test_tuple),
